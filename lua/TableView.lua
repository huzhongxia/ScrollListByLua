...
function TableView:AddItem(index, needRefresh)
...
function TableView:_UpdateContentSize()
    if self.maxCount > 0 then
        local maxPosition = self.cellsOffsets[#self.cellsOffsets] + self.contentSizeExpand + (self.horizontal and self.origin.x or -self.origin.y)
...
-- 假的无限滚动效果 在滚到底的时候往回跳一定数量的cell并把数据做同样的滚动
-- 这个接口返回一次跳的数量和跳到的滚动位置 在外部进行数据变化和位置变化
function TableView:GetPseudoInfiniteMoveCountAndPos()
    local moveUnitCount = self.maxCount - #self.cellsInUse
    local scrollDistance = self.horizontal and (self:GetCurContentSize().x - self:GetViewportSize().x) or (self:GetCurContentSize().y - self:GetViewportSize().y)
    local cellSize = self.horizontal and self:GetCellSize(0).x or self:GetCellSize(0).y
    local curPos = self:GetCurrScrollPos()
    local targetPos = curPos - moveUnitCount * cellSize/scrollDistance
    if targetPos < 0 then
        targetPos = 0
    end
    return moveUnitCount, targetPos
end
